...
RUN yum install -y bash curl tini unzip \
        libuuid && \
...
RUN curl -sLo dd-java-agent.jar 'https://search.maven.org/classic/remote_content?g=com.datadoghq&a=dd-java-agent&v=LATEST' && mv dd-java-agent.jar /tmp/
